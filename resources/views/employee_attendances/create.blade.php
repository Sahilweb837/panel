                    ...
                    <div class="card premium-stat-card p-4">
                        <form method="GET" action="{{ route('employee-attendances.create') }}" id="date-select-form" class="d-flex align-items-end justify-content-between flex-wrap gap-3">
                            ...
                            <div class="d-flex gap-2">
                                <button type="button" onclick="markUnpunchedAbsent()" class="button button-secondary px-3" style="border-color: rgba(220, 53, 69, 0.3) !important; color: #dc3545 !important;" title="Mark staff who haven't punched today as Absent"><i class="fas fa-user-times me-2"></i>Mark Unpunched Absent</button>
                                    ...
                                    @foreach($employees as $employee)
                                         ...
                                         <tr>
                                             ...
                                             <td class="text-center">
                                                 ...
                                                 @if($photo)
                                                     <img id="photo_preview_{{ $employee->id }}" src="{{ asset($photo) }}" style="width: 40px; height: 40px; border-radius: 50%; object-fit: cover; border: 2px solid var(--first-color); margin: 0 auto 5px;" />
                                                 @else
            ...
            timeInputs.forEach(input => {
                ...
                if (!input.value || input.value.trim() === '') {
                    ...
                    if (idMatch && idMatch[1]) {
                        ...
                        const absentRadio = document.getElementById(`status_${employeeId}_absent`);
                        if (absentRadio) {
                            absentRadio.checked = true;
                            // Clear any automatically populated time by handleStatusChange
                            const timeField = document.getElementById(`time_${employeeId}`);
